function HCP_EN_acc_AAvsWA_matchedBehavior(EN_dir, max_seed, num_test_folds, split_AAdir, split_WAdir, ...
    use_seed_bhvr_ldir, out_pCOD, out_corr, mtch_bhvr_ls, full_bhvr_ls, subj_ls)

% HCP_EN_acc_AAvsWA_matchedBehavior(EN_dir, max_seed, num_test_folds, split_AAdir, split_WAdir, ...
%     use_seed_bhvr_ldir, out_pCOD, out_corr, mtch_bhvr_ls, full_bhvr_ls, subj_ls)
%
% Calculate predictive COD and Pearson's correlation accuracies for the matched AA and WA groups separately. 
% The predition was performed using elastic net.
%
% Inputs:
% - EN_dir
%   The top-level output directory of elastic net. This directory contains one subdirectory 
%   for each random data split. Within each random data split folder, there is a subfolder corresponding
%   to each behavioral measures (for which matched AA and WA can be selected).
%
% - max_seed
...
for seed = 1:max_seed
    ...
    for b = 1:n_mtch
        ...
        curr_subfold = fullfile(EN_dir, ['randseed_' num2str(seed)], mtch_bhvr{b}, ...
            ['no_relative_10_fold_sub_list_' mtch_bhvr{b} '.mat']);
        load(curr_subfold)

        curr_result = fullfile(EN_dir, ['randseed_' num2str(seed)], mtch_bhvr{b}, 'optimal_acc', ...
            [mtch_bhvr{b} '_final_acc.mat']);
        ...
        for fold = 1:num_test_folds
            load(fullfile(EN_dir, ['randseed_' num2str(seed)], mtch_bhvr{b}, 'y', ...
            ...
            [~, Aind_fold] = intersect(subjects(sub_fold(fold).fold_index==1), AA_fold.sub_perfold{fold}, 'stable');
            curr_yp_AA = curr_result.y_predict{fold}(Aind_fold);
            yp_AA = [yp_AA; curr_yp_AA];

            [~,~, Aind_match] = intersect(AA_fold.sub_perfold{fold}, subjects, 'stable');
            curr_yt_AA_match = y_resid(Aind_match);
            [~,~, Aind_match_fold] = intersect(AA_fold.sub_perfold{fold}, subjects(sub_fold(fold).fold_index==1), 'stable');
            curr_yp_AA_match = curr_result.y_predict{fold}(Aind_match_fold);
            ...
            [~, Wind_fold] = intersect(subjects(sub_fold(fold).fold_index==1), best_assign{b_ind(b)}{fold}, 'stable');
            curr_yp_WA = curr_result.y_predict{fold}(Wind_fold);
            yp_WA = [yp_WA; curr_yp_WA];

            [~,~, Wind_match] = intersect(best_assign{b_ind(b)}{fold}, subjects, 'stable');
            curr_yt_WA_match = y_resid(Wind_match);
            [~,~, Wind_match_fold] = intersect(best_assign{b_ind(b)}{fold}, subjects(sub_fold(fold).fold_index==1), 'stable');
            curr_yp_WA_match = curr_result.y_predict{fold}(Wind_match_fold);
...
if(~exist(fullfile(EN_dir, 'lists', 'pCOD_union_pos_behaviors.txt'), 'file'))
    mean_COD_comb = nanmean(COD_comb, 1);
    mkdir(fullfile(EN_dir, 'lists'))
    CBIG_cell2text(mtch_bhvr(mean_COD_comb>0), fullfile(EN_dir, 'lists', ...
        'pCOD_comb_pos_behaviors.txt'))

    mean_COD_AA = nanmean(COD_AA, 1);
    CBIG_cell2text(mtch_bhvr(mean_COD_AA>0), fullfile(EN_dir, 'lists', ...
        'pCOD_AA_pos_behaviors.txt'))

    mean_COD_WA = nanmean(COD_WA, 1);
    CBIG_cell2text(mtch_bhvr(mean_COD_WA>0), fullfile(EN_dir, 'lists', ...
        'pCOD_WA_pos_behaviors.txt'))

    CBIG_cell2text(mtch_bhvr( mean_COD_AA>0 | mean_COD_WA>0 ), ...
        fullfile(EN_dir, 'lists', 'pCOD_union_pos_behaviors.txt'))
end

%% check which behavioral measures have positive Pearson's correlation
if(~exist(fullfile(EN_dir, 'lists', 'corr_union_pos_behaviors.txt'), 'file'))
    mean_corr_comb = nanmean(corr_comb, 1);
    CBIG_cell2text(mtch_bhvr(mean_corr_comb>0), fullfile(EN_dir, 'lists', ...
        'corr_comb_pos_behaviors.txt'))

    mean_corr_AA = nanmean(corr_AA, 1);
    CBIG_cell2text(mtch_bhvr(mean_corr_AA>0), fullfile(EN_dir, 'lists', ...
        'corr_AA_pos_behaviors.txt'))

    mean_corr_WA = nanmean(corr_WA, 1);
    CBIG_cell2text(mtch_bhvr(mean_corr_WA>0), fullfile(EN_dir, 'lists', ...
        'corr_WA_pos_behaviors.txt'))

    CBIG_cell2text(mtch_bhvr( mean_corr_AA>0 | mean_corr_WA>0 ), ...
        fullfile(EN_dir, 'lists', 'corr_union_pos_behaviors.txt'))
end
