function HCP_LRR_acc_AAvsWA_matchedBehavior( LRR_dir, max_seed, num_test_folds, split_AAdir, split_WAdir, ...
    use_seed_bhvr_ldir, out_pCOD, out_corr, mtch_bhvr_ls, full_bhvr_ls, subj_ls )

% HCP_KRR_acc_AAvsWA_matchedBehavior( LRR_dir, max_seed, num_test_folds, split_AAdir, split_WAdir, ...
%     use_seed_bhvr_ldir, out_pCOD, out_corr, mtch_bhvr_ls, full_bhvr_ls, subj_ls )
%
% Calculate predictive COD and Pearson's correlation accuracies for the matched AA and WA groups separately. 
% The predition was performed using linear ridge regression.
%
% Inputs:
% - LRR_dir
%   The top-level output directory of linear ridge regression. This directory contains one subdirectory 
...
for seed = 1:max_seed
    ...
    for b = 1:n_mtch
        ...
        curr_subfold = fullfile(LRR_dir, ['randseed_' num2str(seed)], mtch_bhvr{b}, ...
            ['no_relative_10_fold_sub_list_' mtch_bhvr{b} '.mat']);
        load(curr_subfold)

        curr_result = fullfile(LRR_dir, ['randseed_' num2str(seed)], mtch_bhvr{b}, ...
            'results', 'optimal_acc', [mtch_bhvr{b} '.mat']);
        ...
        for fold = 1:num_test_folds
            load(fullfile(LRR_dir, ['randseed_' num2str(seed)], mtch_bhvr{b}, 'y', ...
            ...
            [~, Aind_in_test] = intersect(sub_fold(fold).subject_list, AA_fold.sub_perfold{fold}, 'stable');
            curr_yp_AA = curr_result.y_predict{fold}(Aind_in_test);
            yp_AA = [yp_AA; curr_yp_AA];

            [~, ~, Aind_match] = intersect(AA_fold.sub_perfold{fold}, subjects, 'stable');
            curr_yt_AA_match = y_resid(Aind_match);
            [~, ~, Aind_match_in_test] = intersect(AA_fold.sub_perfold{fold}, sub_fold(fold).subject_list, 'stable');
            curr_yp_AA_match = curr_result.y_predict{fold}(Aind_match_in_test);
            ...
            [~, Wind_in_test] = intersect(sub_fold(fold).subject_list, ...
                best_assign{b_ind(b)}{fold}, 'stable');
            curr_yp_WA = curr_result.y_predict{fold}(Wind_in_test);
            yp_WA = [yp_WA; curr_yp_WA];

            [~, ~, Wind_match] = intersect(best_assign{b_ind(b)}{fold}, subjects, 'stable');
            curr_yt_WA_match = y_resid(Wind_match);
            [~, ~, Wind_match_in_test] = intersect(best_assign{b_ind(b)}{fold}, ...
                sub_fold(fold).subject_list, 'stable');
            curr_yp_WA_match = curr_result.y_predict{fold}(Wind_match_in_test);
...
if(~exist(fullfile(LRR_dir, 'lists', 'pCOD_union_pos_behaviors.txt'), 'file'))
    mean_COD_comb = nanmean(COD_comb, 1);
    mkdir(fullfile(LRR_dir, 'lists'))
    CBIG_cell2text(mtch_bhvr(mean_COD_comb>0), fullfile(LRR_dir, 'lists', ...
        'pCOD_comb_pos_behaviors.txt'))

    mean_COD_AA = nanmean(COD_AA, 1);
    CBIG_cell2text(mtch_bhvr(mean_COD_AA>0), fullfile(LRR_dir, 'lists', ...
        'pCOD_AA_pos_behaviors.txt'))

    mean_COD_WA = nanmean(COD_WA, 1);
    CBIG_cell2text(mtch_bhvr(mean_COD_WA>0), fullfile(LRR_dir, 'lists', ...
        'pCOD_WA_pos_behaviors.txt'))

    CBIG_cell2text(mtch_bhvr( mean_COD_AA>0 | mean_COD_WA>0 ), ...
        fullfile(LRR_dir, 'lists', 'pCOD_union_pos_behaviors.txt'))
end

%% check which behavioral measures have positive Pearson's correlation
if(~exist(fullfile(LRR_dir, 'lists', 'corr_union_pos_behaviors.txt'), 'file'))
    mean_corr_comb = nanmean(corr_comb, 1);
    CBIG_cell2text(mtch_bhvr(mean_corr_comb>0), fullfile(LRR_dir, 'lists', ...
        'corr_comb_pos_behaviors.txt'))

    mean_corr_AA = nanmean(corr_AA, 1);
    CBIG_cell2text(mtch_bhvr(mean_corr_AA>0), fullfile(LRR_dir, 'lists', ...
        'corr_AA_pos_behaviors.txt'))

    mean_corr_WA = nanmean(corr_WA, 1);
    CBIG_cell2text(mtch_bhvr(mean_corr_WA>0), fullfile(LRR_dir, 'lists', ...
        'corr_WA_pos_behaviors.txt'))

    CBIG_cell2text(mtch_bhvr( mean_corr_AA>0 | mean_corr_WA>0 ), ...
        fullfile(LRR_dir, 'lists', 'corr_union_pos_behaviors.txt'))
end
